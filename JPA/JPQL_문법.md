...
- SQL을 추강화한 JPQL은 객체 지향 쿼리 언어 제공
- JPQL - 엔티티객체를 대상으로 쿼리
- SQL - 데이터베이스 테이블을 대상으르 쿼리
...
- 컴파일 시점에 문법 오류를 찾을 수 있고 동적 쿼리 작성 편리
...
![image.png](/JPA/images/img17.png)
...
1. SELECT **m** FROM Member m → 엔티티 프로젝션
    ...
    - `.getResultList()` 로 나온 데이터들도 영속성 컨텍스트로 관리
2. SELECT m.team FROM Member m → 엔티티 프로젝션
   ...
   ![image.png](/JPA/images/img18.png)

    - 조인이 에측 가능할 수 있지만, 조인은 웬만하면 명시적으로 표현 하는것이 좋다.
...
- 나이가 평균보다 많은 회원
  ...
  → 서브쿼리와 원래 쿼리랑 m, m2로 아예 관계 없음 ⇒ 성능 더 높음
...
- 타입 표현
    ...
    - Boolean: TRUE, FALSEENUM: jpabook.MemberType.Admin (패키지명 포함)
...
- `SIZE`, `INDEX` (JPA 용도)
    - SIZE : 컬렉션의 크기 출력
    - INDEX : List 값 타입일 때 컬렉션 위치 값 구할 때 사용 (권장X)